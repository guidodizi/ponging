...
if (isMobile()) {
  box.addEventListener("touchmove", moveCursor);
}
...
function moveCursor(event) {
  if (player) {
    let pos_y;
    // Mobile
    if (event instanceof TouchEvent) pos_y = event.touches[0].clientY;
    // Desktop
    else pos_y = event.clientY;

    // set position on middle of cursor
    pos_y = pos_y - cursor_height / 2 - box.offsetTop;
    // top of board
    if (pos_y <= 0) pos_y = 0;
    // bottom of board
    else if (pos_y + cursor_height >= box.offsetHeight) pos_y = box.offsetHeight - cursor_height;

    cursor_1.style.top = `${pos_y}px`;
    socket.emit("coord", { player, coord: pos_y });
  }
}
